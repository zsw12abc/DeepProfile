...
DeepProfile 是一个 Chrome 浏览器插件，利用 AI 技术（OpenAI, Gemini, DeepSeek, Ollama, Qwen）分析社交媒体用户的近期动态，生成多维度的性格与领域偏好画像，辅助理性交流。最新版本 v0.9.0 重点优化了用户画像分析的准确度，并确保AI摘要与标签分数保持一致性。
...
*   **本地存储**: 所有 API 密钥和历史记录仅存储在您的浏览器本地，不会上传到任何服务器。
*   **数据安全**: 您的 API 密钥使用浏览器的加密存储机制保护。
*   **无数据收集**: 我们不收集、存储或分享任何用户数据。
*   **透明处理**: 分析时仅向您配置的 AI 服务发送公开的社交媒体内容。

完整的隐私政策请参见 [PRIVACY_POLICY.md](./PRIVACY_POLICY.md)。
