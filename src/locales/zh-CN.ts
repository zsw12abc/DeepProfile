...
## 当前版本: v1.0.1 (Stable) - 小版本更新

### 🚀 小版本更新 (v1.0.1)
- **AI 回复风格新增**: 增加 4 种新风格选项：\`巨魔风格 (Troll)\`、\`贴吧大神风格\`、\`古早公知风格\`、\`当代衍生变体\`
- **多语言风格映射**: 新风格支持中英文风格名映射（如 \`巨魔风格 (Troll)\` / \`Troll\`），跨平台可识别
- **跨语言回复保持**: 风格不改变语言策略，仍按上下文/语言检测结果生成对应语言回复
- **安全约束补强**: 为高冲突风格增加约束，禁止辱骂、歧视、威胁与人身攻击，避免恶意引战输出
- **测试覆盖补充**: 新增风格提示词注入测试，验证约束规则稳定生效

### 🎉 正式版发布 (v1.0.0)
- **版本里程碑**: 从 Beta 进入 Stable，完成 1.0.0 正式发布
- **全量类型检查收敛**: 清理全量 TypeScript 类型错误，\`typecheck\` 全量通过
- **CI门禁升级**: CI 从信息提示升级为强制全量类型检查，确保持续质量
- **稳定性增强**: 持续保留并强化内存管理、监听器清理与缓存回收策略
- **多平台能力完善**: 知乎 / Reddit / Twitter(X) / Quora 分析链路保持可用并通过回归测试
...
### 小版本更新 (v1.0.1) - AI 回复风格扩展与多语言映射
- ✅ **新增4种回复风格**: \`巨魔风格 (Troll)\`、\`贴吧大神风格\`、\`古早公知风格\`、\`当代衍生变体\`
- ✅ **中英文风格名兼容**: 支持风格名中英双写与跨平台识别
- ✅ **语言策略稳定**: 输出语言仍严格跟随上下文检测/用户偏好语言，不因风格切换而漂移
- ✅ **高风险风格约束**: 增加“禁止辱骂/人身攻击”等规则，降低引战与攻击性输出
- ✅ **自动化测试补充**: 增加新风格提示词与约束校验测试

### 正式版发布 (v1.0.0) - 工程质量收敛与稳定性增强
- ✅ **版本里程碑**: 从 Beta 进入 Stable，完成 1.0.0 正式发布
- ✅ **全量类型检查收敛**: 清理全量 TypeScript 类型错误，\`typecheck\` 全量通过
- ✅ **CI门禁升级**: CI 从信息提示升级为强制全量类型检查，确保持续质量
- ✅ **稳定性增强**: 持续保留并强化内存管理、监听器清理与缓存回收策略
