...
describe("VersionInfo", () => {
  it("renders version info section", () => {
    render(
      <VersionInfo
        ...
        versionHistory="Test version history content"
      />,
    );
  ...
  describe("getVersion", () => {
    ...
    it("returns fallback version when manifest unavailable", () => {
      ...
      const version = getVersion();
      expect(version).toBe("1.0.1"); // This should be the fallback
  ...
  describe("fetchChangelogContent", () => {
    it("returns Chinese changelog and version history content when language is zh-CN", async () => {
      ...
      expect(content.changelog).toBe(
        "# DeepProfile 当前版本更新日志\n\n## 当前版本: v0.6.2 (Beta)",
      );
      expect(content.versionHistory).toBe(
        "# DeepProfile 版本历史\n\n### v0.6.1 (2024-01-09) - 实时保存设置",
      );
    });
