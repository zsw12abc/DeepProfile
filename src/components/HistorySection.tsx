...
export const HistorySection: React.FC<HistorySectionProps> = ({
  ...
  return (
    <Card
      title={I18nService.t("settings_history")}
      ...
      {loadingHistory ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {historyRecords.map((userRecord) => (
            <details
              key={`${userRecord.platform}-${userRecord.userId}`}
              style={{
                padding: "16px",
                ...
                transition: "all 0.2s",
                boxShadow:
                  "var(--theme-shadow-small, 0 2px 4px rgba(0,0,0,0.02))",
              }}
            >
              <summary
                style={{
              ...
              >
                ...
                >
                  <span
                    style={{
                      ...
                      borderRadius: "4px",
                      backgroundColor:
                        userRecord.platform === "zhihu"
                          ? "var(--theme-primary, #e1f0fa)"
                          : userRecord.platform === "twitter"
                            ? "var(--theme-accent, #1DA1F2)"
                            : userRecord.platform === "quora"
                              ? "var(--theme-success, #b9261c)"
                              : "var(--theme-warning, #ffedd5)",
                      color:
                        userRecord.platform === "zhihu"
                ...
                <button
                  onClick={(e) => {
                    e.preventDefault();
                    handleDeleteUser(userRecord.userId, userRecord.platform);
                  }}
                  style={{
                    padding: "8px",
              ...
              >
                {Object.values(userRecord.profiles || {}).map((profile) => {
                  ...
                  const categoryName = TopicService.getCategoryName(
                    profile.category as MacroCategory,
                  );
                  const summary = profile.profileData.summary;
                  const labels =
                    profile.profileData.value_orientation ||
                    profile.profileData.political_leaning ||
                    [];
                  const visibleLabels = labels.filter(
                    (item: { label: string; score: number }) =>
                      Math.abs(item.score) > 1e-6,
                  );
                    ...
                    >
                      ...
                      <div
                        style={{
                          marginTop: "12px",
                          paddingTop: "12px",
                          borderTop: "1px dashed var(--theme-border, #e2e8f0)",
                        }}
                      >
                        <p
                          ...
                          {visibleLabels.map(
                            (
                              ...
                              const normalizedScore = Math.max(
                                -1,
                                Math.min(1, item.score),
                              );

                              // 计算百分比（取分数绝对值并转换为百分比）
                              const percentage =
                                Math.abs(normalizedScore) * 100;
                              ...
                              const leftColor =
                                item.score < 0
                                  ? "var(--theme-error, #e74c3c)"
                                  : "var(--theme-border, #e0e0e0)";
                              const rightColor =
                                item.score > 0
                                  ? "var(--theme-primary, #3498db)"
                                  : "var(--theme-border, #e0e0e0)";
