...
const CommentAnalysisPanel = ({
    ...
    const analyze = async () => {
      try {
        ...
        const expandBtn = Array.from(
          containerElement.querySelectorAll("div, button"),
        ).find(
          (el) =>
            (el.textContent?.includes("点击查看全部评论") ||
              el.textContent?.includes("展开更多评论")) &&
        ...
        if (expandBtn) {
          ...
          await new Promise<void>((resolve) => {
            ...
            const interval = setInterval(() => {
              attempts++;
              const contentCount =
                containerElement.querySelectorAll(".CommentContent").length;
              const btnStillVisible =
                document.body.contains(expandBtn) &&
        ...
        const contentElements =
          containerElement.querySelectorAll(".CommentContent");
        ...
        contentElements.forEach((contentEl, index) => {
          ...
          const itemContainer =
            contentEl.closest("[data-id]") ||
            contentEl.closest("li") ||
            contentEl.parentElement?.parentElement;
          ...
          if (itemContainer) {
            const authorEl =
              itemContainer.querySelector(".UserLink-link") ||
        ...
        console.log(
          `[DeepProfile] Extracted ${comments.length} comments from container.`,
        );

        if (comments.length < 3) {
          throw new Error(
            `${I18nService.t("not_enough_comments")} (${comments.length}${I18nService.t("comment_analysis_instruction")}`,
          );
        }
        ...
        try {
          ...
          if (contentContainer) {
            const richContent =
              contentContainer.querySelector(".RichContent-inner") ||
              contentContainer.querySelector(".Post-RichText");
            if (richContent) {
            ...
            if (
              window.location.href.includes("/answer/") ||
          ...
          if (contextContent) {
            console.log(
              `[DeepProfile] Extracted context content length: ${contextContent.length}`,
            );
          }
        } catch (e) {
        ...
        try {
          ...
          if (response.success) {
            console.log(
              "[DeepProfile] Received analysis result:",
              response.data,
            );
          ...
          if (
            ...
            console.error(
              "[DeepProfile] Extension context invalidated, please refresh the page",
            );
  ...
  if (error) {
    if (
      ...
      error === I18nService.t("extension_context_invalidated")
    ) {
      return (
        <div
          style={{
        ...
        >
          <strong>
            {I18nService.t("extension_context_invalidated_title")}：
          </strong>{" "}
          {I18nService.t("extension_context_invalidated_desc")}
        </div>
      );
    } else {
      return (
    ...
    >
      ...
      >
        <div style={{ flex: 1 }}>
          ...
          >
            ...
            >
              {I18nService.t("comment_analysis_ai_generated")}
            </span>
          </h3>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        >
          <div
            style={{
              width: `${(result.stance_ratio?.support || 0) * 100}%`,
              background: "linear-gradient(90deg, #22c55e, #16a34a)",
            }}
            title={`${I18nService.t("sentiment_support")} ${Math.round((result.stance_ratio?.support || 0) * 100)}%`}
          />
          <div
            style={{
              width: `${(result.stance_ratio?.neutral || 0) * 100}%`,
              background: "linear-gradient(90deg, #f59e0b, #fbbf24)",
            }}
            title={`${I18nService.t("sentiment_neutral")} ${Math.round((result.stance_ratio?.neutral || 0) * 100)}%`}
          />
          <div
            style={{
              width: `${(result.stance_ratio?.oppose || 0) * 100}%`,
              background: "linear-gradient(90deg, #ef4444, #f97316)",
            }}
            title={`${I18nService.t("sentiment_oppose")} ${Math.round((result.stance_ratio?.oppose || 0) * 100)}%`}
          />
        </div>
        <div
          style={{
        ...
        >
          <span style={{ display: "flex", alignItems: "center" }}>
            ...
            ></span>
            {I18nService.t("sentiment_support")}{" "}
            {Math.round((result.stance_ratio?.support || 0) * 100)}%
          </span>
          <span style={{ display: "flex", alignItems: "center" }}>
            <span
              style={{
            ...
            ></span>
            {I18nService.t("sentiment_neutral")}{" "}
            {Math.round((result.stance_ratio?.neutral || 0) * 100)}%
          </span>
          <span style={{ display: "flex", alignItems: "center" }}>
            <span
              style={{
            ...
            ></span>
            {I18nService.t("sentiment_oppose")}{" "}
            {Math.round((result.stance_ratio?.oppose || 0) * 100)}%
          </span>
        </div>
      </div>
      ...
      <div>
        ...
        >
          {isExpanded
            ? I18nService.t("collapse_key_points")
            : I18nService.t("expand_key_points")}
          <svg
            width="12"
            height="12"
            viewBox="0 0 24 24"
        ...
        {isExpanded && (
          ...
          >
            ...
            {result.deep_analysis && result.deep_analysis.has_fallacy && (
              ...
              >
                <div
                  style={{ fontWeight: 600, marginBottom: 4, color: "#0f172a" }}
                >
                  {I18nService.t("deep_insight")}
                </div>
                <div style={{ fontSize: 12, color: "#64748b" }}>
                  {`${I18nService.t("logic_fallacy")}: ${result.deep_analysis.fallacy_type || I18nService.t("unknown_type")}`}
                  {result.deep_analysis.example &&
                    ` (${I18nService.t("example_quote")}: "${result.deep_analysis.example}")`}
                </div>
              </div>
            )}
          </div>
...
const ZhihuComments = () => {
  useEffect(() => {
    let isEnabled = false;
    let storageListenerRef: ((changes: any, area: string) => void) | null =
      null;
    ...
    const cleanup = () => {
      document
        .querySelectorAll(".deep-profile-summary-btn")
        .forEach((el) => el.remove());
      document
        .querySelectorAll(".deep-profile-embedded-panel")
        .forEach((el) => el.remove());
    };
    ...
    const injectButton = () => {
      ...
      containers.forEach((container) => {
        ...
        // 优先使用更具体的选择器
        header =
          container.querySelector(".css-1onritu") ||
        ...
        if (!header) {
          ...
          for (const child of children) {
            if (
              child.innerText.includes("默认") &&
              child.innerText.includes("最新")
            ) {
              header = child;
              break;
            }
        ...
        // 在 header 内部找到排序容器
        sortContainer = Array.from(header.querySelectorAll("div")).find(
          (d) => d.innerText.includes("默认") && d.innerText.includes("最新"),
        ) as HTMLElement;
        ...
        btn.style.background =
          "linear-gradient(135deg, rgba(37, 99, 235, 0.14), rgba(34, 211, 238, 0.14))";
    ...
    const checkConfig = async () => {
      try {
        ...
        const commentEnabled =
          config?.platformConfigs?.zhihu?.commentAnalysisEnabled ??
        ...
        console.warn(
          "[DeepProfile] Failed to read config for comment analysis:",
          e,
        );
      }
    };
