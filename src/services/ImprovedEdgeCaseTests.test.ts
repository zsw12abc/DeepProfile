...
describe('Improved Edge Case Tests', () => {
  describe('LLMService Edge Cases', () => {
    ...
    it('should handle invalid score values with custom validation', () => {
      ...
      const testScoreNormalization = (score: any, expectedMin: number, expectedMax: number) => {
        let normalizedScore = score;
        if (typeof score !== 'number' || isNaN(score) || score === Infinity || score === -Infinity) {
          normalizedScore = 0.5;
        } else {
  ...
  describe('TopicService Edge Cases', () => {
    ...
    it('should calculate keyword confidence properly', () => {
      ...
      const confidence = (TopicService as any).calculateKeywordConfidence(text, category);
  ...
  describe('Confidence-based Classification', () => {
    it('should return confidence scores with classification', async () => {
      const result = await TopicService.classifyWithConfidence('这是一个关于人工智能技术的文章');
