...
export class QuoraClient {
  ...
  static async fetchUserContent(username: string, limit: number = 15, context?: string): Promise<FetchResult> {
    try {
      console.log(`Attempting to fetch Quora content for user: ${username} via content script`);
      ...
      const contentFromTab = await this.fetchContentFromTab(username, limit, quoraAnswersUrl);
        ...
        chrome.tabs.query({active: true, currentWindow: true}, (tabs) => {
          ...
          if (activeTab && activeTab.id) {
            ...
            const responseCallback = (response: any) => {
              if (response && response.type === 'QUORA_CONTENT_RESPONSE' && response.requestId === requestId) {
                // 清理消息监听器
  ...
  // 获取特定内容的详细信息
  static async fetchDetailContent(id: string, type: 'answer' | 'question' = 'answer'): Promise<string | null> {
    try {
      // 由于API限制，返回空值
      return null;
    } catch (error) {
