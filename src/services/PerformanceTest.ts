...
export class PerformanceTest {
  ...
  private static async testMode(mode: 'fast' | 'balanced' | 'deep', content: string, category: MacroCategory) {
    ...
    try {
      ...
      if (result.content) {
        console.log(`    分析结果预览: ${result.content.summary?.substring(0, 50) || 'N/A'}`);
      }
    } catch (error) {
  ...
  static async batchPerformanceTest() {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      // 测试快速模式
      const fastResult = await this.measureModePerformance('fast', testContent, 'technology');
      results.push({ mode: 'fast', duration: fastResult });
      
      // 测试平衡模式
      const balancedResult = await this.measureModePerformance('balanced', testContent, 'technology');
  ...
  private static printPerformanceStats(results: { mode: string; duration: number }[]) {
    console.log('\\n性能统计结果:');
    
    const modes = ['fast', 'balanced'];
    for (const mode of modes) {
      const modeResults = results.filter(r => r.mode === mode && r.duration > 0);
