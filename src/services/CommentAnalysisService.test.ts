...
describe("CommentAnalysisService", () => {
  ...
  describe("analyzeComments", () => {
    it("should analyze comments correctly", async () => {
      ...
      vi.mocked(LLMService.generateRawText).mockResolvedValue(
      ...
      const result = await CommentAnalysisService.analyzeComments(
        comments as any,
        "Test Topic",
        "Test Content",
        "zhihu",
    ...
    it("should handle empty comments", async () => {
      await expect(
        CommentAnalysisService.analyzeComments([], "Test Topic"),
      ).rejects.toThrow("没有找到可分析的评论");
    });
    ...
    it("should handle LLM parsing error", async () => {
      ...
      await expect(
        CommentAnalysisService.analyzeComments(comments as any, "Test Topic"),
      ).rejects.toThrow();
    });
