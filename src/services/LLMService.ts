...
export class LLMService {
  ...
  private static sanitizeInputText(text: string): string {
    ...
    const sensitiveWords: Record<string, string> = {
      ...
      种族灭绝: "genocide",
      镇压: "suppression",
      反送中: "Anti-Extradition Bill Movement",
      光复香港: "Liberate HK",
      时代革命: "Revolution of Our Times",
      香港警察: "HK Police",
      黑警: "black police",
      巴勒斯坦: "Palestine",
      以色列: "Israel",
      ...
      "Great Firewall": "GFW",
      feminism: "gender equality movement",
      LGBTQ: "LGBTQ",
      gay: "homosexual",
      lesbian: "homosexual",
      transgender: "trans",
    ...
    switch (type) {
      case "openai":
        return new LangChainProvider(
          ...
          customModel ||
            process.env.PLASMO_PUBLIC_OPENAI_MODEL ||
            "gpt-3.5-turbo",
        );
      case "deepseek":
        return new LangChainProvider(
          ...
          customModel ||
            process.env.PLASMO_PUBLIC_DEEPSEEK_MODEL ||
            "deepseek-chat",
        );
      case "qwen":
        return new LangChainProvider(
      ...
      case "gemini":
        return new LangChainProvider(
          ...
          customModel ||
            process.env.PLASMO_PUBLIC_GEMINI_MODEL ||
            "gemini-1.5-flash",
        );
      case "ollama":
...
class LangChainProvider implements LLMProvider {
  ...
  async generateProfile(
    ...
    try {
      ...
      const chain = RunnableSequence.from([() => messages, this.llm, parser]);
      ...
      const result = await withRetry(
        async (attempt) => {
          try {
            ...
            Logger.error(
              `LangChain API Error (attempt ${attempt + 1}):`,
              error,
            );
            ...
            if (attempt === 0 && this.shouldRetryOnError(error)) {
              ...
              const corrected = await this.retryWithErrorFeedback(
                text,
                mode,
                category,
                error.message,
      ...
      if (
        parsedContent.value_orientation &&
        Array.isArray(parsedContent.value_orientation)
      ) {
        ...
        parsedContent.value_orientation = Array.from(labelMap.entries()).map(
          ([label, score]) => ({
            label,
            score,
          }),
      ...
      const consistentProfile = {
        ...parsedContent,
        evidence:
          ConsistencyService.validateAndFixEvidenceConsistency(parsedContent)
            .evidence,
      };
      const labelIds = normalizeLabelIdList(
        LabelService.getInstance().getLabelsForContext(category, mode),
      );
      ...
      if (config.enableDebug) {
        Logger.info(
          "【LANGCHAIN LABEL SCORES】LLM Scores：",
          alignedProfile.value_orientation,
        );
        Logger.info(
          "【CONSISTENCY REPORT】",
          ConsistencyService.generateConsistencyReport(alignedProfile),
        );
      }
  ...
  // 基于错误反馈进行重试
  private async retryWithErrorFeedback(
    text: string,
    mode: AnalysisMode,
    category: MacroCategory,
    errorMessage: string,
...
Now, please analyze the following text again:` +
    ...
    try {
      ...
      const chain = RunnableSequence.from([() => messages, this.llm]);
...
class OllamaProvider implements LLMProvider {
  ...
  async generateProfile(
    ...
    const prompt =
      LLMService.getSystemPrompt(mode, category, text) + "\n\n" + text;
      ...
      const data = await withRetry(
        async (attempt) => {
          ...
          if (!response.ok) {
            ...
            Logger.error(
              `Ollama API Error (attempt ${attempt + 1}): ${response.status}`,
              errorText,
            );
