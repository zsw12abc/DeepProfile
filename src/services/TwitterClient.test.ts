...
describe("TwitterClient", () => {
  ...
  describe("fetchDetailContent", () => {
    it("should return null (current API limitation)", async () => {
      const resultTweet = await TwitterClient.fetchDetailContent(
        "12345",
        "tweet",
      );
      const resultReply = await TwitterClient.fetchDetailContent(
        "12345",
        "reply",
      );
  ...
  describe("fetchUserContent", () => {
    ...
    it("should fetch content through chrome API when available", async () => {
      ...
      const fetchSpy = vi
        .spyOn(TwitterClient as any, "fetchContentFromTab")
        .mockResolvedValue(mockContent);

      const result = await TwitterClient.fetchUserContent(
        "testuser",
        15,
        "test context",
      );
  ...
  describe("sortByRelevance (private method)", () => {
      ...
      const items: ZhihuContent[] = [
        {
          id: "1",
          title: "I like apples",
          excerpt: "Apple is good",
          content: "Very good",
          created_time: 10000000000,
          url: "",
          type: "article",
          action_type: "created",
        } as ZhihuContent,
    ...
    it("should return original items if context is empty", () => {
      const items: ZhihuContent[] = [
        { id: "1", title: "Test" } as ZhihuContent,
      ];
      const sortByRelevance = (TwitterClient as any).sortByRelevance;
  ...
  describe("fetchContentFromTab (chrome API simulation)", () => {
    it("should handle no active tab", async () => {
      ...
      const result = await (TwitterClient as any).fetchContentFromTab(
        "testuser",
        10,
        "url",
      );
    ...
    it("should resolve with content when message connects successfully", async () => {
      ...
      const fetchPromise = (TwitterClient as any).fetchContentFromTab(
        "testuser",
        10,
        "url",
      );
      ...
      const sendMessageCall = (global.chrome.tabs.sendMessage as any).mock
        .calls[0];
