...
// 将标签名称解析为左右选项
export function parseLabelName(labelName: string): {
  left: string;
  right: string;
} {
...
export function calculateFinalLabel(
  labelName: string,
  score: number,
...
export function getRelevantLabelsByTopic(topic: string) {
  ...
  if (
    topicLower.includes("政治") ||
    topicLower.includes("左") ||
    topicLower.includes("右") ||
    topicLower.includes("自由") ||
    topicLower.includes("保守") ||
    topicLower.includes("威权") ||
    topicLower.includes("社会主义") ||
    topicLower.includes("资本主义") ||
    topicLower.includes("民族") ||
    topicLower.includes("全球")
  ) {
    relevantCategories.push("politics", "economy", "society");
  } else if (
    topicLower.includes("经济") ||
    topicLower.includes("市场") ||
    topicLower.includes("政府") ||
    topicLower.includes("资本主义") ||
    topicLower.includes("社会主义") ||
    topicLower.includes("财富") ||
    topicLower.includes("分配")
  ) {
    relevantCategories.push("economy", "politics");
  } else if (
    topicLower.includes("社会") ||
    topicLower.includes("个人") ||
    topicLower.includes("集体") ||
    topicLower.includes("传统") ||
    topicLower.includes("进步") ||
    topicLower.includes("文化")
  ) {
    relevantCategories.push("society", "culture", "politics");
  } else if (
    topicLower.includes("科技") ||
    topicLower.includes("技术") ||
    topicLower.includes("创新") ||
    ...
    topicLower.includes("文化") ||
    topicLower.includes("本土") ||
    topicLower.includes("全球") ||
    topicLower.includes("物质") ||
    topicLower.includes("精神")
  ) {
    relevantCategories.push("culture", "society");
  } else if (
    topicLower.includes("环境") ||
    topicLower.includes("生态") ||
    topicLower.includes("发展") ||
    topicLower.includes("保护")
  ) {
...
export function filterLabelsByTopic(
  ...
  const filtered = labels.filter((item) => {
    ...
    return relevantLabels.some((rlId) => {
      ...
      // 检查名称是否包含
      if (
        item.label.includes(rlInfo.name) ||
        rlInfo.name.includes(item.label)
      ) {
        return true;
      }
  ...
  if (filtered.length === 0 && labels.length > 0) {
    ...
    const topicLower = topic.toLowerCase();
    if (
      topicLower.includes("娱乐") ||
      topicLower.includes("游戏") ||
      topicLower.includes("动漫") ||
      topicLower.includes("电影") ||
      topicLower.includes("音乐") ||
      topicLower.includes("体育")
    ) {
      return [];
    }
...
export function getMostRelevantLabelsByTopic(topic: string): string[] {
    ...
    for (const label of category.labels) {
      ...
      if (
        topicLower
          ...
          .some(
            (word) =>
              labelNameLower.includes(word) ||
              (word.length > 2 && labelNameLower.includes(word)),
          )
      ) {
        categoryScore++;
      }
  ...
  if (matchedCategories.length === 0) {
    ...
    if (
      ...
      topicLower.includes("创新") ||
      topicLower.includes("ai") ||
      topicLower.includes("软件") ||
      topicLower.includes("硬件") ||
      topicLower.includes("数码") ||
      topicLower.includes("编程") ||
      topicLower.includes("算法")
    ) {
      ...
      topicLower.includes("音乐") ||
      topicLower.includes("体育") ||
      topicLower.includes("明星") ||
      topicLower.includes("综艺")
    ) {
      // 对于娱乐类话题，不返回政治相关标签，只返回通用性格特征
      return []; // 娱乐话题可能不需要政治倾向标签
    } else if (
      topicLower.includes("财经") ||
      topicLower.includes("投资") ||
      topicLower.includes("股票") ||
      topicLower.includes("金融") ||
      topicLower.includes("商业") ||
      topicLower.includes("创业")
    ) {
