...
describe('ExampleService', () => {
  ...
  it('should retrieve relevant examples for politics category', () => {
    const inputContent = "政府应该减少对市场的干预，让企业和个人有更多的自由来创造财富";
    const category: MacroCategory = 'politics';
    
    const relevantExamples = exampleService.getRelevantExamples(inputContent, category, 'balanced', 2);
  ...
  it('should calculate relevance scores', () => {
    ...
    const relevanceScore = (exampleService as any).calculateRelevanceScore(inputContent, exampleContent);
  ...
  it('should format examples as prompt', () => {
    ...
    const category: MacroCategory = 'politics';
    const examples = exampleService.getRelevantExamples(inputContent, category, 'balanced', 1);
    
    const formattedPrompt = exampleService.formatExamplesAsPrompt(examples, 'balanced');
