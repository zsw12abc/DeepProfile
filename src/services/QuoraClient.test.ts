...
describe("QuoraClient", () => {
  ...
  describe("fetchDetailContent", () => {
    it("should return null (current API limitation)", async () => {
      const resultAnswer = await QuoraClient.fetchDetailContent(
        "12345",
        "answer",
      );
      const resultQuestion = await QuoraClient.fetchDetailContent(
        "12345",
        "question",
      );
  ...
  describe("fetchUserContent", () => {
    ...
    it("should fetch content through chrome API when available", async () => {
      ...
      const fetchSpy = vi
        .spyOn(QuoraClient as any, "fetchContentFromTab")
        .mockResolvedValue(mockContent);

      const result = await QuoraClient.fetchUserContent(
        "testuser",
        15,
        "test context",
      );
      ...
      expect(fetchSpy).toHaveBeenCalledWith(
        "testuser",
        15,
        "https://www.quora.com/profile/testuser/answers",
      );
  ...
  describe("fetchContentFromTab (chrome API simulation)", () => {
    it("should handle no active tab", async () => {
      ...
      const result = await (QuoraClient as any).fetchContentFromTab(
        "testuser",
        10,
        "url",
      );
    ...
    it("should resolve with content when message connects successfully", async () => {
      ...
      const fetchPromise = (QuoraClient as any).fetchContentFromTab(
        "testuser",
        10,
        "url",
      );
