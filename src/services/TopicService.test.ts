...
describe("TopicService", () => {
  ...
  describe("classify", () => {
    ...
    it("should classify technology related text", () => {
      expect(TopicService.classify("这是一篇关于科技的文章")).toBe(
        "technology",
      );
    ...
    it("should classify environment related text", () => {
      expect(TopicService.classify("这是一篇关于环境的文章")).toBe(
        "environment",
      );
    ...
    it("should classify entertainment related text", () => {
      expect(TopicService.classify("这是一篇关于娱乐的文章")).toBe(
        "entertainment",
      );
    ...
    it("should classify lifestyle_career related text", () => {
      expect(TopicService.classify("这是一篇关于生活的文章")).toBe(
        "lifestyle_career",
      );
    ...
    it("should return general for unknown text", () => {
      ...
      expect(TopicService.classify("random text with no clear category")).toBe(
        "general",
      );
    });
  });
  ...
  describe("getCategoryName", () => {
    it("should return localized category names", () => {
      ...
      expect(typeof TopicService.getCategoryName("entertainment")).toBe(
        "string",
      );
      expect(typeof TopicService.getCategoryName("lifestyle_career")).toBe(
        "string",
      );
  ...
  describe("classifyWithLLM", () => {
    it("should return general on LLM error", async () => {
      vi.spyOn(LLMService, "generateRawText").mockRejectedValue(
        new Error("API Error"),
      );
    ...
    it("should return general on invalid LLM response", async () => {
      vi.spyOn(LLMService, "generateRawText").mockResolvedValue(
        "invalid_category",
      );
    ...
    it("should return valid category on successful LLM response", async () => {
      ...
      const result = await TopicService.classifyWithLLM(
    ...
    it("should cache LLM classification results", async () => {
      const spy = vi
        ...
        .mockResolvedValue("technology");

      const first = await TopicService.classifyWithLLM(
        "AI technology discussion",
      );
      const second = await TopicService.classifyWithLLM(
        "AI technology discussion",
      );
