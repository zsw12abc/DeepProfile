...
describe("ConsistencyService", () => {
  ...
  describe("validateAndFixSummaryConsistency", () => {
    ...
    it("should not modify summary when labels are already mentioned", () => {
      const profileWithLabelReference = {
        ...sampleProfile,
        summary:
          "This user shows strong individualistic tendencies and urban preferences.",
      };

      const result = ConsistencyService.validateAndFixSummaryConsistency(
        profileWithLabelReference,
      );
  ...
  describe("validateAndFixEvidenceConsistency", () => {
    it("should not add template evidence for high-score labels without supporting evidence", () => {
      ...
      const addedEvidence = result.evidence!.filter(
        (e) => e.source_id === "consistency_fix",
      );
    ...
    it("should preserve existing evidence without adding template evidence", () => {
      ...
      const templateEvidence = result.evidence!.filter(
        (e) =>
          e.source_id === "consistency_fix" ||
          e.source_id === "medium_score_evidence",
      );
  ...
  describe("generateConsistencyReport", () => {
    it("should generate a readable consistency report", () => {
      const report =
        ConsistencyService.generateConsistencyReport(sampleProfile);
    ...
    it("should handle incomplete profile data", () => {
      ...
      const report = ConsistencyService.generateConsistencyReport(
  ...
  describe("enforceSummaryAlignment", () => {
    it("should append alignment hints for balanced mode when missing", () => {
      ...
      const result = ConsistencyService.enforceSummaryAlignment(
        profile,
        "balanced",
      );
  ...
  describe("normalizeScores", () => {
    it("should clamp scores and fill missing labels", () => {
      ...
      const result = ConsistencyService.normalizeScores(profile, [
        "ideology",
        "authority",
        "change",
      ]);
      ...
      expect(
        result.value_orientation?.find((l) => l.label === "ideology")?.score,
      ).toBe(1);
      expect(
        result.value_orientation?.find((l) => l.label === "authority")?.score,
      ).toBe(-1);
      expect(
        result.value_orientation?.find((l) => l.label === "change")?.score,
      ).toBe(0);
    });
  });
  ...
  describe("adjustScoresByEvidence", () => {
    it("should dampen high scores without evidence support", () => {
      ...
      const result = ConsistencyService.adjustScoresByEvidence(profile, [
